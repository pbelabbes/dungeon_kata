...
function countWords(message: string): Map<string, number> {
  const wordsMap = new Map<string, number>();
  const wordArray = message.replace(/./g, '').split(' ');
  const wordsSet = new Set(wordArray);
  for (const word in wordsSet) {
    const nbOccurrences: number = wordArray.find(e => e === word)?.length ?? 0;
    wordsMap.set(word, nbOccurrences);
  }
